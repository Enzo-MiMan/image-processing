...
def convolve(filter, mat, padding, strides):  # convolve(filter,mat,[0,0,padding[0],padding[1]],strides)
    """
...
def _2_dim_divided_convolve(filter, mat):  # linear_Gaussian_filter_5, sq_img_gx
    '''
...
if __name__ == '__main__':
    ...
    for i in pics:
        if i[-5:]=='.jpeg':
            original_img = plt.imread(pic_path+i)

            # plt.imshow(original_img)
            # plt.axis('off')
            # plt.show()

            img = original_img.mean(axis=-1)  # 彩色图片转单通道灰度图
            ...
            final_img = Sign(original_img,score,[12,12])
